...
import { X, Save, SquarePen, Trash, Plus } from 'lucide-vue-next'
...
const debouncedFetch = debounce(() => fetchUsers(1), 300)
...
const createRecord = async () => {
    if (!creatingRecord.value?.name) {
        errorMessages.value.new = 'Name is required'
        return
    }

    try {
        const response = await axios.post(`${url}/admin/properties`, {
                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <tr v-if="creatingRecord">
                            <td></td>
                            <td>
                                <input type="text" v-model="creatingRecord.name" class="form-control"
                                    ref="newInputRef" />
                        ...
                        <tr v-for="(record, index) in records" :key="record.id">
                            ...
                            <td>
                                ...
                                <div v-if="errorMessages[record.id]" class="text-danger mt-1"
                        ...
                        <ul class="pagination justify-content-md-end mb-0">
                            <li v-for="(link, index) in pagination.links" :key="index" class="page-item"
                                ...
                                <a class="page-link" href="#" v-html="link.label"
                                    @click.prevent="goToPage(link.url)"></a>
                            </li>
                        </ul>
                    </nav>
                </div>
